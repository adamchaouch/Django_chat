...
class Server(models.Model):
    ...
    owner = models.ForeignKey(
        settings.AUTH_USER, on_delete=models.CASCADE, related_name="server_owner"
    )
    category = models.ForeignKey(
        Category, on_delete=models.CASCADE, related_name="server_category"
    )
    description = models.CharField(max_length=255, null=True)
    member = models.ManyToManyField(settings.AUTH_USER)
...
class Channel(models.Model):
    name = models.CharField(max_length=100)
    owner = models.ForeignKey(
        settings.AUTH_USER, on_delete=models.CASCADE, related_name="channel_owner"
    )
    ...
    def save(self, *args, **kwargs):
        if self.id:
            ...
            if exist.banner != self.banner:
                exist.banner.delete(save=False)
